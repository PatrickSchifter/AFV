...
const ComboFamilia = ({width, margin, description, disabled, dispatch, value}) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const [selected, setSelected] = useState(value === null ? '' : value)
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `familia`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((familia)=>{
                array.push({value: familia.IDFamilia, label: familia.Descricao})
            })
            setOptions(array);
