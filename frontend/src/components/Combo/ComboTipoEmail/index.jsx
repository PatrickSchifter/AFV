...
const ComboTipoEmail = ({width, margin, description, disabled, dispatch, value}) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `tipo-email`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((tipoEmail)=>{
                array.push({value: tipoEmail.IDTabGeral, label: tipoEmail.Descricao})
            })
            setOptions(array);
