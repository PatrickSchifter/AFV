...
const ComboProduto = ({width, margin, description, disabled, dispatch, value}) => {
    ...
    const url = input === '' ? process.env.REACT_APP_API_URL + `produto?pagina=1` : process.env.REACT_APP_API_URL + `produto?pesquisa=${input}`
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((produto)=>{
                array.push({value: produto.IDProduto, label: produto.SKU + ' - ' + produto.DesERP}) 
            })
            setOptions(array);
