...
const ComboLinhaProduto = ({width, margin, description, disabled, dispatch, value   }) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const [selected, setSelected] = useState(value === null ? '' : value)
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `linha-produto`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((linha)=>{
                array.push({value: linha.IDLinha, label: linha.DesLinha})
            })
            setOptions(array);
...
export default ComboLinhaProduto;
