...
const ComboSubGrupo = ({width, margin, description, disabled, dispatch, value}) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `sub-grupo`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((subGrupo)=>{
                array.push({value: subGrupo.IDSubGrupo, label: subGrupo.Descricao})
            })
            setOptions(array);
