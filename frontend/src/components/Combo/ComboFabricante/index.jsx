...
const ComboFabricante = ({width, margin, description, disabled, dispatch, value}) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `fabricante`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((fabricante)=>{
                array.push({value: fabricante.IDFabricante, label: fabricante.Descricao})
            })
            setOptions(array);
...
export default ComboFabricante;
