...
const ComboGrupo = ({width, margin, description, disabled, dispatch, value}) => {
    const [options, setOptions] = useState([]); 
    const [isLoading, setIsLoading] = useState(false); 
    const [input, setInput] = useState('');
    const [selected, setSelected] = useState(value === null ? '' : value)
    const {setIsAuthenticated} = useContext(AuthContext);
    const url = process.env.REACT_APP_API_URL + `grupo`;
    ...
    useEffect(() => {
        async function obtemDados(){
            ...
            response.data.map((grupo)=>{
                array.push({value: grupo.IDGrupo, label: grupo.Descricao})
            })
            setOptions(array);
